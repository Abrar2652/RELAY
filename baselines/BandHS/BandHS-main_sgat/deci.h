...
{
  public:
    Decimation(lit **ls_var_lit, int *ls_var_lit_count, lit **ls_clause_lit, long long *ls_org_clause_weight, long long ls_top_clause_weight, double *gnn_pred);
...
Decimation::Decimation(lit **ls_var_lit, int *ls_var_lit_count, lit **ls_clause_lit, long long *ls_org_clause_weight, long long ls_top_clause_weight, double *ls_gnn_pred)
{
    var_lit = ls_var_lit;
...
{
    double gnn_value = gnn_pred[v];
    if (gnn_value > (double)rand() / (double)RAND_MAX)
        return 1;
    else
        return 0;
    //return rand() % 2;
