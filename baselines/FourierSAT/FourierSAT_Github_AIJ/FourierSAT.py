...
def verify_sol(formula, x, klist, weight, ctype):
    ...
    for l_list in clauses:
        is_unsat = False
        if ctype[i] == 'c':
            satisfied = any((j > 0 and x[abs(j)-1] > 0) or (j < 0 and x[abs(j)-1] < 0) for j in l_list)
            if not satisfied:
                is_unsat = True
        elif ctype[i] == 'x':
            if prod(j/abs(j)*x[abs(j)-1] for j in l_list) > 0:
                is_unsat = True
        elif ctype[i] == 'n':
            v0 = l_list[0]/abs(l_list[0])*x[abs(l_list[0])-1]
            if all(j/abs(j)*x[abs(j)-1] == v0 for j in l_list):
                is_unsat = True
        if is_unsat:
            ...
            for cpu_index in range(cpus):
                ...
                if num_of_unsat == 0:
                    ...
                    if num_of_unsat < best_unsat_num:
                        ...
                        if ismaxsat == 1:
                            print('o ' + repr(int(weight_of_unsat)))
            if verbose == 1:
