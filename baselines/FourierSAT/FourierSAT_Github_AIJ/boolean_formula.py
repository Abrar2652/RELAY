    ...
    def read_DIMACS(filename, include_missing_vars=False):
        ...
        with open(filename, 'r') as f:
            for line in f:
                ...
                if not split:
                    continue
                # Detect Weighted MaxSAT line: p wcnf ...
                if split[0] == 'p' and len(split) >= 5 and split[1] == 'wcnf':
                    result._iswcnf = 1
                    num_vars = int(split[2])
                    result._variables = [i + 1 for i in range(num_vars)]
                    top_weight = float(split[4])
                    continue

                # If we are in WCNF mode and the line starts with a digit, parse it as a weighted clause
                if result._iswcnf == 1 and (split[0].isdigit() or isfloat(split[0])):
                    w = float(split[0])
                    literals = list(map(int, split[1:-1]))
                    if not literals:
                        continue
                    # Mark hard vs soft
                    ctype = 'c'
                    coefs = [1 for _ in literals]
                    result.add_clause(literals, 1, ctype, w, coefs, '>=')
                    continue

                # ...existing DIMACS parsing code for regular CNF...
