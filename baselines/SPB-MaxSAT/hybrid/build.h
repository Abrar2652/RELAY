...
void SPBMaxSAT::build_instance(char *filename)
{
    ...
    // Modification Start
    printf("c Loading GNN prediction file...\n");

    istringstream iss_gnn;
    string line_gnn;
    double iss_db_gnn;
    ifstream infile_gnn("prediction_file.csv");

    for (v = 1; v <= num_vars; ++v){
        getline(infile_gnn, line_gnn);

        iss_gnn.clear();
        iss_gnn.str(line_gnn);

        iss_gnn >> iss_db_gnn;
        gnn_pred[v] = iss_db_gnn;
    }
    infile_gnn.close();
    printf("c GNN prediction file loaded\n");
    // Modification End
...
void SPBMaxSAT::allocate_memory()
{
    ...
    gnn_pred = new double[malloc_var_length];
